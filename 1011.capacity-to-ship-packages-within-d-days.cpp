...
public:
    void count_days(vector<int>& w, int& days, int& max_w, int& min_w){
        ...
        if(day > days) min_w = mid_w+1;
        else  max_w = mid_w;

        // cout << max_w << ":" << min_w << ":" << mid_w << "," << day << "\n";
        if(min_w < max_w) count_days(w, days, max_w, min_w);
    }
    ...
    int shipWithinDays(vector<int>& weights, int days) {
         ...
         * @brief 
         * Solution: 
         * Accepted, 50ms(84.86%), 26.1MB(77.07%)
         * Time Complexity: O(NlogW), which N = weights' size, W = (max_w - min_w)
         * 1. Beacuse we don't know how to split the conveyor belt, we need to 
         *    find the target weight for the limit of once shipment.
         * 2. The target weight must between the whole weight of package and 
         *    the maxinum weight package.
         * 3. Therefore, check the target weight, if we can split the conveyor
         *    belt for "days" by the target weight, that's the answer.
         * 4. To find the target weight, we can use binary search instead of one
         *    by one.
         * 5. Optimized: 
         *    (1) If days = 1, the answer must be the whole weight of package.
         *    (2) If days = weight.size(), the answer must be the maxinum weight 
         *        package.
         * 6. For some reason, the "days" might not just fit the "day", however, 
         *    if "day" is smaller than "days", the answer can definitely can use
         *    for "days".
         *    Therefore, we could not just check if(day == days), but need to 
         *    return the "min_w" when "min_w >= max_w" instead.
         */
        int max_w = 0;
        int min_w = 0;
        for(auto w:weights){
        ...
        if(days == 1) return max_w;
        else if(weights.size() > days) count_days(weights, days, max_w, min_w);
        return min_w;
    }
};
// @lc code=end
